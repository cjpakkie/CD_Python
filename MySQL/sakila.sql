...
-- Your query should return customer first name, last name, email, and address.
SELECT customer.store_id, city.city_id, CONCAT_WS(' ', customer.first_name, customer.last_name) AS customer_name, customer.email, address.address
FROM customer
...
-- Question 6 --
-- What query would you run to get all the actors that joined in the 
-- film_id = 369? Your query should return the film_id, title, actor_id, 
-- and actor_name.
...
-- Question 8 --
-- What query would you run to get all the action films which are joined 
...
SELECT CONCAT_WS(' ', actor.first_name, actor.last_name) AS actor_name, film.title, film.description, film.release_year,film.rating, film.special_features, category.name
FROM film
JOIN film_category ON film.film_id = film_category.film_id
JOIN category ON category.category_id = film_category.category_id
JOIN film_actor ON film.film_id = film_actor.film_id
